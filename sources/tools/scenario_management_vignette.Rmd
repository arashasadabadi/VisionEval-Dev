...
`JSON` files are easily consumed by software, but difficult to edit manually. The scenario management tools provide an alternative to editing the JSON file, by allowing you to edit them in the form of a simple database table in CSV (Comma-Separated Values) format. 

Here's how the tools work.

The tools are loaded automatically as VisionEval helpers, from the tools directory. The following steps presume that you have installed VisionEval and started it (seeing the "Welcome to VisionEval!" message).
...
prev_df <- jsonlite::read_json(file.path(example_file_dir, "scenario_config.json"),
    simplifyVector = TRUE) %>%
  dplyr::arrange(NAME)
new_df <- jsonlite::read_json(file.path(new_json_file_dir, "scenario_config.json"),
    simplifyVector = TRUE) %>%
...
Finally, to rebuild the HTML version of this document from the Rmarkdown (.Rmd), you can simply do this after you start VisionEval:

```{r rebuild-html, eval=FALSE}
setwd(file.path(ve.root,"tools"))
rmarkdown::render("scenario_management_vignette.Rmd",output_format="html_document")
```
